...
class ParserModel(nn.Module):
    ...
    def embedding_lookup(self, w):
        ...
        x = torch.index_select(self.embeddings, 0, w.flatten()) # (batch_size * n_features, embed_size)
        x = x.view(len(w), -1)  # (batch_size, n_features * embed_size)
        return x
    ...
    def forward(self, w):
        ...
        x_hidden = self.dropout(torch.relu(x @ self.embed_to_hidden_weight + self.embed_to_hidden_bias))
        logits = x_hidden @ self.hidden_to_logits_weight + self.hidden_to_logits_bias
